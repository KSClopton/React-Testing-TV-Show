...
import { render, waitFor, screen, getByPlaceholderText, findAllByRole, findAllByTestId, queryByTestId } from '@testing-library/react'
...
test("This will test the Async API call", async () => {
    // Arrange
    mockFetchShow.mockResolvedValueOnce(mockData)
    const {findAllByRole, findAllByTestId, findByText} = render(<App />)
...
test("This will test the episodes component", () => {

    const {queryAllByTestId, rerender} = render(<Episodes episodes={[]}/>)

    const emptyArr = queryAllByTestId(/episode/i)
    expect(emptyArr).toHaveLength(0)

    rerender(<Episodes episodes={mockArrData}/>)

    const notEmptyArr = queryAllByTestId(/episode/i)
    expect(notEmptyArr).toHaveLength(1)

})
// render nothing as the episode prop to check stuff


// then rerender with data to make sure it's got stuff
